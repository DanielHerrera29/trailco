...
import { ArrowRight, Truck, ShieldCheck, FileCheck, ChevronLeft, ChevronRight } from "lucide-react"
...
const services = [
  {
    ...
    images: ["/images/Tractomula.jpeg", "/images/CamaBaja.jpeg"],
    features: [
...
export function Services() {
  ...
  function nextImage(serviceIndex: number, total: number) {
    setImageIndexes((prev) => ({
      ...prev,
      [serviceIndex]: ((prev[serviceIndex] || 0) + 1) % total,
    }))
  }

  function prevImage(serviceIndex: number, total: number) {
    setImageIndexes((prev) => ({
      ...prev,
      [serviceIndex]: ((prev[serviceIndex] || 0) - 1 + total) % total,
    }))
  }
      ...
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="mt-20 flex flex-col gap-24">
          {services.map((service, index) => {
            const isCarousel = "images" in service
            const currentIdx = imageIndexes[index] || 0
            const totalImages = isCarousel ? (service as typeof services[0] & { images: string[] }).images.length : 1
            const src = isCarousel
              ? (service as typeof services[0] & { images: string[] }).images[currentIdx]
              : service.image
                        ...
                        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-10 flex gap-2">
                          {Array.from({ length: totalImages }).map((_, i) => (
                            <button
                              key={i}
                              onClick={() =>
                                setImageIndexes((prev) => ({ ...prev, [index]: i }))
                              }
